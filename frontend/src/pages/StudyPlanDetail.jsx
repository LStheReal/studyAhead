    ...
    <div className="p-4 space-y-6">
      ...
      {activeTab === 'overview' ? (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          {plan.status === 'generating' ? (
          ...
          ) : (!preAssessment || preAssessment.status !== 'completed') ? (
            ...
            <div className="card bg-gradient-to-r from-blue-600 to-indigo-700 text-white transform hover:scale-[1.01] transition-all cursor-pointer"
              onClick={() => {
                const nextTask = tasks.filter(t => !t.completion_status)[0]
                handleStartMode(nextTask.mode, nextTask.id)
              }}>
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-blue-200 text-xs font-bold uppercase tracking-wider">Up Next</span>
                  <h2 className="text-2xl font-bold mb-1 flex items-center gap-2">
                    {tasks.filter(t => !t.completion_status)[0].title}
                  </h2>
                  <p className="text-blue-100 opacity-90 text-sm">
                    {tasks.filter(t => !t.completion_status)[0].estimated_minutes} min • Day {tasks.filter(t => !t.completion_status)[0].day_number}
                  </p>
                </div>
